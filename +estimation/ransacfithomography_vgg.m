...
function [H, inliers] = ransacfithomography_vgg(x1, x2, t, skip_non_lin)
    if nargin == 3
        skip_non_lin = 0;
    end
    ...
    [x1, T1] = vgg.normalise2dpts(x1);
    [x2, T2] = vgg.normalise2dpts(x2);
    ...
    % be inliers.
    Hlin = vgg.vgg_H_from_x_lin(x1(:,inliers), x2(:,inliers));
    if ~skip_non_lin
        H = vgg.vgg_H_from_x_nonlin(Hlin,x1(:,inliers), x2(:,inliers));
    end
...
function [inliers, H] = homogdist2d(H, x, t)
    ...
    x1     = vgg.hnormalise(x1);
    x2     = vgg.hnormalise(x2);     
    Hx1    = vgg.hnormalise(Hx1);
    invHx2 = vgg.hnormalise(invHx2); 
...
function r = isdegenerate(x)
    ...
    r = ...
    vgg.iscolinear(x1(:,1),x1(:,2),x1(:,3)) | ...
    vgg.iscolinear(x1(:,1),x1(:,2),x1(:,4)) | ...
    vgg.iscolinear(x1(:,1),x1(:,3),x1(:,4)) | ...
    vgg.iscolinear(x1(:,2),x1(:,3),x1(:,4)) | ...
    vgg.iscolinear(x2(:,1),x2(:,2),x2(:,3)) | ...
    vgg.iscolinear(x2(:,1),x2(:,2),x2(:,4)) | ...
    vgg.iscolinear(x2(:,1),x2(:,3),x2(:,4)) | ...
    vgg.iscolinear(x2(:,2),x2(:,3),x2(:,4));
    
function H = wrap_vgg_homography2d(x)

    H = vgg.vgg_H_from_x_lin(x(1:3,:),x(4:6,:));
